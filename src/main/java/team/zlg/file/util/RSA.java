...
//            String pqeFileName = "E:\\Graduation Project\\xiguafile\\src\\main\\java\\team\\zlg\\file\\util\\pqe.txt";
    ...
    private static boolean isPrime(BigInteger p) {
        if (p.compareTo(BigInteger.valueOf(2)) == -1) {// 小于2直接返回false
            return false;
        }
        if ((p.compareTo(BigInteger.valueOf(2)) != 0) && (p.remainder(BigInteger.valueOf(2)).compareTo(BigInteger.ZERO) == 0)) {// 不等于2且是偶数直接返回false
            return false;
        }
        ...
        for (int i = 0; i < 5; i++) {// 判断的轮数，精度、轮数和时间三者之间成正比关系
            ...
            while (!((j == 0 && z.equals(BigInteger.ONE)) || z.equals(p_1))) {
                ...
                z = RSA.expMod(z, BigInteger.valueOf(2), p);
            }
        }
